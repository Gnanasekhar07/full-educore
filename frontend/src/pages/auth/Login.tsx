...
const Login = () => {
    ...
    const handleGoogleSuccess = async (tokenResponse: any) => {
        ...
        setError('');
        try {
            // Get the ID token from the access token
            const res = await fetch('https://www.googleapis.com/oauth2/v3/userinfo', {
                headers: { Authorization: `Bearer ${tokenResponse.access_token}` }
            });
            const userInfo = await res.json();

            // Use the sub as a temporary credential identifier
            ...
            >
                <div className="glass-card p-10">
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-3 py-3 px-4 border border-slate-200 rounded-xl bg-white hover:bg-slate-50 transition-all font-medium text-slate-700 shadow-sm mb-6 group"
                    >
                        {googleLoading ? (
