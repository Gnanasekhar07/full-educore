...
export const createQuiz = async (req: any, res: Response) => {
    try {
        console.log("=== INCOMING CREATE QUIZ REQUEST ===");
        ...
        if (course.teacherId !== req.user.id && req.user.role !== 'ADMIN') {
            console.log("Forbidden: course.teacherId vs req.user.id", course.teacherId, req.user.id);
...
export const getQuizzesByCourse = async (req: any, res: Response) => {
    try {
...
export const getUpcomingQuizzes = async (req: any, res: Response) => {
    try {
